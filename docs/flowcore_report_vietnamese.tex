...
\begin{itemize}
    ...
    \item Bổ sung SPI Rule Engine với double-buffer rule table, cơ chế reload bằng \texttt{SIGUSR1} và đường xử lý worker dùng snapshot metadata để tránh đọc nhầm slot khi flow bị reuse.
    \item Cung cấp thống kê per-lcore, phân loại HTTP/HTTPS/DNS/TCP/UDP/OTHER, thống kê flow lifecycle, drop, overload replacement và throughput.
    \item Bổ sung cơ chế stateful replacement khi Flow Table gần đầy, gồm pressure mode, victim cache và retry add flow mới sau khi evict flow cũ.
    \item Xây dựng bộ kiểm thử unit, functional bằng PCAP PMD, kiểm thử overload với hash nhỏ và các kịch bản đo baseline phần mềm.
\end{itemize}
...
\begin{itemize}
    ...
    \item Parser hiện đọc header từ segment đầu tiên của mbuf và không xử lý đầy đủ fragmented/scattered RX path.
\end{itemize}
...
Ứng dụng hiện yêu cầu tối thiểu hai DPDK ports khả dụng vì mã nguồn cố định \texttt{PORT\_IN = 0} và \texttt{PORT\_OUT = 1}. Từ phiên bản hiện tại, mọi bước \texttt{rx/tx queue setup} trong \texttt{ports\_init()} đều được kiểm tra lỗi và fail-fast nếu topology port không đáp ứng mô hình 1 dispatcher + 4 worker TX queue.
...
\hline
\texttt{FLOW\_PRESSURE\_*} & 92/96/99\% & Các watermark điều khiển pressure maintenance và replacement, target sau evict là 92\% \\
\hline
\texttt{FLOW\_VICTIM\_CACHE\_SIZE} & 8192 & Số candidate victim tối đa được cache cho dispatcher \\
\hline
\texttt{FLOW\_REPLACEMENT\_RETRIES} & 8 & Số lần dispatcher thử evict/reclaim/retry khi add flow mới thất bại \\
\hline
...
\texttt{hot[]} chứa các trường bị chạm thường xuyên trong đường xử lý nhanh. Dispatcher cần \texttt{worker\_id} để route packet sang ring đúng worker và cập nhật \texttt{last\_seen} cho aging. Worker cần \texttt{flow\_gen} để kiểm tra metadata còn hợp lệ trước khi copy \texttt{cold[]} thành snapshot cục bộ.
...
Một chi tiết quan trọng khác là cơ chế publish slot bằng \texttt{last\_seen}. Khi dispatcher tạo flow mới, key được add vào hash trước, sau đó dispatcher mới fill \texttt{cold[]}, tăng \texttt{flow\_gen}, gán \texttt{worker\_id} và cuối cùng mới store \texttt{last\_seen}. Trong thời gian rất ngắn giữa add key và store \texttt{last\_seen}, slot được coi là chưa publish; aging, pressure maintenance và replacement đều bỏ qua slot có \texttt{last\_seen == 0}. Cách này tránh trường hợp stats thread nhìn thấy key mới nhưng metadata side-array chưa hoàn chỉnh rồi xóa hoặc chọn nhầm làm victim.
...
\begin{enumerate}
    ...
    \item Với flow miss: gọi \texttt{rte\_hash\_add\_key()}, khởi tạo cold data, tăng \texttt{flow\_gen}, gán worker, store \texttt{last\_seen} cuối cùng và tăng \texttt{flows\_created}.
    \item Nếu add fail: evict victim từ victim cache hoặc emergency scan nhỏ, reclaim một phần rồi retry add flow mới.
...
Mỗi worker dequeue từ ring của mình. Worker kiểm tra \texttt{flow\_gen} để xác định metadata còn trỏ đúng flow slot hay không. Nếu hợp lệ, worker copy \texttt{cold[flow\_idx]} sang một snapshot cục bộ, sau đó kiểm tra lại \texttt{flow\_gen}. Nếu generation vẫn khớp, worker match SPI trên snapshot này. Nếu slot đã bị reuse hoặc generation đổi trong lúc copy, worker parse lại packet sang \texttt{flow\_cold\_data} tạm thời rồi match SPI trực tiếp.
...
Các action \texttt{LOG} và \texttt{COUNT} được normalize thành \texttt{FORWARD} để tránh logging đồng bộ trên hot path. Hệ thống hỗ trợ reload runtime bằng \texttt{SIGUSR1}. Rule table dùng double-buffering: rule mới được nạp vào bảng inactive, sau đó con trỏ active được đổi bằng atomic store. Worker luôn match rule trên snapshot cold-data hiện tại, nhờ đó rule reload có hiệu lực trên packet tiếp theo mà không cần ghi cache quyết định vào flow slot.
...
Điểm phòng vệ thứ hai là \texttt{last\_seen == 0} được dùng như marker slot chưa publish. Dispatcher luôn ghi \texttt{last\_seen} sau khi các trường còn lại đã hợp lệ. Vì vậy stats/aging thread không được aging hoặc replacement một slot mà dispatcher mới add key nhưng chưa fill xong metadata. Nếu muốn thiết kế chặt hơn nữa, có thể thêm field trạng thái riêng như \texttt{FLOW\_PUBLISHING/FLOW\_ACTIVE}, nhưng với mục tiêu giữ hot entry nhỏ, việc tận dụng \texttt{last\_seen} là tradeoff hợp lý.
...
\subsection{SPI matching trên snapshot và reload rule}

Worker hiện match SPI trên snapshot \texttt{flow\_cold\_data} thay vì ghi cache action vào flow slot. Cách này tốn thêm chi phí match rule trên mỗi packet, nhưng tránh một race quan trọng: packet cũ có thể đang nằm trong worker ring trong khi flow slot bị replacement reuse cho flow mới. Nếu worker ghi \texttt{spi\_action}/\texttt{action\_version} vào slot sau khi slot đã đổi chủ, nó có thể làm bẩn state của flow mới. Snapshot local giúp packet đang xử lý có dữ liệu nhất quán mà không cần pin/refcount trên hot path.
...
Tradeoff là linear match hiện tại có chi phí phụ thuộc số rule. Với \texttt{SPI\_MAX\_RULES = 256}, chi phí này chấp nhận được cho mini project và đổi lại concurrency an toàn hơn. Nếu rule set lớn, hướng tối ưu tiếp theo là phân nhóm rule theo protocol/port, dùng DPDK \texttt{rte\_acl}, hoặc khôi phục per-flow action cache nhưng phải bổ sung pin/refcount hoặc state atomic để không ghi nhầm vào slot đã reuse.
...
\subsection{Stateful replacement khi overload flow}

Khi số flow tiến gần giới hạn của Flow Table, hệ thống không chuyển sang xử lý stateless. Lý do là yêu cầu chính của đề tài là quản lý Flow Table: flow mới cần có bản ghi, có \texttt{worker\_id}, \texttt{last\_seen}, vòng đời create/update/delete và thống kê active/created/deleted. Vì vậy cơ chế overload được thiết kế theo hướng stateful replacement: flow cũ bị thay thế để nhường chỗ cho flow mới.

Stats/aging thread định kỳ tính pressure mode dựa trên số active flow. Khi vượt watermark, thread này scan thêm một phần hash table, evict flow cũ theo hướng idle-first và đưa candidate vào victim cache. Dispatcher chỉ dùng victim cache trong cold path của flow mới: nếu \texttt{rte\_hash\_add\_key()} fail, dispatcher evict một victim, reclaim một phần bằng QSBR, rồi retry add flow mới. Packet hiện tại sau đó được xử lý với flow entry mới.

Victim cache hiện là một \texttt{rte\_ring} SP/SC. Khi enqueue, code pack \texttt{position} và \texttt{flow\_gen} vào một object 64 bit; khi dequeue, key được dựng lại từ \texttt{cold[position]}. Trước khi xóa, code lookup lại key và kiểm tra position, generation và \texttt{last\_seen} để chắc rằng slot chưa đổi chủ và đã publish xong. Nếu candidate đã stale, dispatcher hoặc stats thread bỏ candidate đó và thử candidate khác. Cơ chế này làm replacement an toàn hơn trong bối cảnh aging, add flow mới và worker ring có thể chạy song song.

Tradeoff của replacement là capacity cứng không tăng: bảng 1M entry vẫn chỉ quản lý khoảng 1M flow state tại một thời điểm. Cơ chế này giúp hệ thống chịu flow churn tốt hơn và ưu tiên flow mới, nhưng flow cũ bị evict sẽ mất state. Nếu packet của flow cũ quay lại, nó lookup miss và được tạo lại như một flow mới. Nếu workload có hơn 1M flow đều active thật sự, giải pháp đúng là tăng capacity, shard Flow Table hoặc dùng RSS/multi-dispatcher để partition state.
...
\begin{itemize}
    ...
    \item Active flows, created flows, deleted flows tổng, timeout delete và pressure evict.
    \item Flow creation/deletion rate và tách riêng tốc độ timeout delete / pressure evict.
    \item SPI forwarded/dropped, rule version và Rule Checks. Counter \texttt{spi\_rule\_checks} đếm số lần worker chạy SPI check trên cold-data snapshot hoặc parse fallback.
    \item Ring drop, TX drop, hash add failure.
    \item Aging scan/timeout-seen/timeout-delete/reclaim rate.
...
\hline
Flow affinity & \texttt{worker\_id} lưu trong \texttt{hot[]} và được lookup lại cho packet cùng flow & Hoàn thành \\
\hline
Phân tải 4--8 worker & Hiện cấu hình 4 worker cố định & Hoàn thành mức 4 worker \\
\hline
Flow aging/timeout & Stats thread gọi aging tick, xóa batch, RCU reclaim & Hoàn thành, timeout hiện là 5 giây \\
\hline
Thống kê realtime & In PPS, Mbps, active flow, drop, protocol, per-worker & Hoàn thành \\
\hline
SPI Rule Engine & Rule file, wildcard, FORWARD/DROP, snapshot matching, reload runtime & Hoàn thành phần chính và một phần nâng cao \\
\hline
Overload replacement & Pressure mode, victim cache, retry add flow mới sau khi evict victim & Đã bổ sung theo hướng stateful replacement \\
\hline
...
Repository có bốn nhóm kiểm thử:
...
\begin{itemize}
    ...
    \item Overload test C: build hash nhỏ để kiểm tra pressure mode, victim cache, unpublished-slot guard và retry add sau replacement.
...
\hline
Overload replacement & Hash nhỏ gần đầy vẫn nhận flow mới bằng cách evict victim hợp lệ & Replacement counters tăng, add flow mới thành công sau evict/reclaim \\
\hline
\end{tabularx}
\end{table}
...
\begin{itemize}
    ...
    \item Flow generation giúp worker phát hiện slot đã bị reuse sau khi aging hoặc replacement xóa flow.
    \item Rule reload không dừng datapath; worker match packet tiếp theo với active rule table mới.
...
\begin{itemize}
    ...
    \item Chưa hỗ trợ IPv6, fragmented IPv4 đầy đủ, scattered RX/jumbo path hoặc non-TCP/UDP ở worker path.
    ...
    \item Replacement ưu tiên flow mới nhưng không làm capacity cứng vượt quá khoảng 1 triệu active state.
...
\bibitem{spi}
Repository FlowCore, \textit{rules.cfg}, \textit{include/spi\_engine.h} và \textit{src/spi\_engine.c}, thiết kế SPI Rule Engine, wildcard rule và hot reload.

\bibitem{defense}
Repository FlowCore, \textit{docs/defense\_guide\_current.md}, hướng dẫn bảo vệ chi tiết theo source code hiện tại.

\bibitem{tests}
Repository FlowCore, \textit{tests/README.md}, \textit{tests/flowcore\_functional\_tests.c} và \textit{tests/flow\_table\_overload\_tests.c}, chiến lược kiểm thử software-only với PCAP PMD và kiểm thử overload.
