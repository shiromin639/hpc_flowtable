...
\hline
\texttt{FLOW\_PRESSURE\_*} & 85/90/95\% & Các watermark điều khiển pressure maintenance và replacement \\
\hline
...
\begin{enumerate}
    ...
    \item Nếu add fail hoặc active flow ở mức critical: evict victim từ victim cache hoặc emergency scan nhỏ, reclaim một phần rồi retry add flow mới.
...
Stats/aging thread định kỳ tính pressure mode dựa trên số active flow. Khi vượt watermark, thread này scan thêm một phần hash table, evict flow cũ theo hướng idle-first và đưa candidate vào victim cache. Dispatcher chỉ dùng victim cache trong cold path của flow mới: nếu \texttt{rte\_hash\_add\_key()} fail hoặc active flow đang ở mức critical, dispatcher evict một victim, reclaim một phần bằng QSBR, rồi retry add flow mới. Packet hiện tại sau đó được xử lý với flow entry mới.

Victim cache không chỉ lưu key mà còn lưu \texttt{position}, \texttt{flow\_gen} và \texttt{last\_seen} của candidate. Trước khi xóa, code lookup lại key và kiểm tra các giá trị này để chắc rằng slot chưa đổi chủ và đã publish xong. Nếu candidate đã stale, dispatcher hoặc stats thread bỏ candidate đó và thử candidate khác. Cơ chế này làm replacement an toàn hơn trong bối cảnh aging, add flow mới và worker ring có thể chạy song song.
...
\begin{itemize}
    ...
    \item SPI forwarded/dropped, rule version, Rule Matches. Tên field trong code còn là \texttt{spi\_rule\_revalidations}, nhưng ý nghĩa hiện tại là số lần worker match SPI sau khi đã bỏ cache action per-flow.
